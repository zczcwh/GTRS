...
def prepare_network(args, load_dir='', is_train=True):
    ...
    if is_train or load_dir:
        ...
        if cfg.MODEL.name == 'GTRS_net':
            model = models.GTRS_net.get_model(num_joint=main_dataset.joint_num, graph_adj=main_dataset.graph_Adj)
        elif cfg.MODEL.name == 'PAM':
            model = models.PAM.get_model(main_dataset.joint_num, embed_dim=128,
                                                  graph_adj=main_dataset.graph_Adj,  pretrained=cfg.MODEL.posenet_pretrained)
    ...
    if load_dir and (not is_train or args.resume_training):
        ...
        checkpoint = load_checkpoint(load_dir=load_dir, pick_best=(cfg.MODEL.name == 'PAM'))
...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for i, (inputs, targets, meta) in enumerate(batch_generator):
            ...
            # model
            pred_mesh, lift_pose = self.model(input_pose)  # B x 6890 x 3
            ...
            loss3 = 0
            loss = loss1 + loss2 + loss3 + loss4 + 0.5*loss5
